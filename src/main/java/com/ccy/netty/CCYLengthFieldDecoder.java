 ...
 */
public class CCYLengthFieldDecoder extends ByteToMessageDecoder {
    ...
    public CCYLengthFieldDecoder(int maxFrameLength, int lengthFieldOffset, int lengthFieldLength, int lengthAdjustment, int initialBytesToStrip) {
        this(maxFrameLength, lengthFieldOffset, lengthFieldLength, lengthAdjustment, initialBytesToStrip, true);
    }

    public CCYLengthFieldDecoder(int maxFrameLength, int lengthFieldOffset, int lengthFieldLength, int lengthAdjustment, int initialBytesToStrip, boolean failFast) {
    ...
    public CCYLengthFieldDecoder(ByteOrder byteOrder, int maxFrameLength, int lengthFieldOffset, int lengthFieldLength, int lengthAdjustment, int initialBytesToStrip, boolean failFast) {
        if (byteOrder == null) {
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
        ...
        if (in.readableBytes() < this.lengthFieldEndOffset) {
            ...
            if (frameLength < 0L) {
                ...
                frameLength += 3; // 王磊的数据帧长度不包括后面的两位校验位和以为结束位
                if (frameLength < (long) this.lengthFieldEndOffset) {
                    ...
                    if (in.readableBytes() < frameLengthInt) {
                        ...
                        return new CCYCollectedData(frame);
                    }
                }
            }
        }
